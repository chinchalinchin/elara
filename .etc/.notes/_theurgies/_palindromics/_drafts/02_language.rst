...
5. ":math:`\leftarrow`" is used to represent the operation of assignment in algorithms.
...
The domain of discourse is composed of *Strings*. A String will be represented as follows, 
    ...
    1. String (:math:`s_1, s_2, s_3, ...`): A lowercase English :math:`s` with a subscript denotes a String. Often the subscript will be dropped and :math:`s` will be used. The letter :math:`t, u, v, w` are also reserved for Strings.

A String is regarded as a linguistic artifact or inscription that is defined by its *Characters* and their *ordering*. It is assumed if one knows which Characters are in a String and in what order they occur, then one has all the information necessary to completely determine the String. This notion is made more precise below with the introduction of several core definitions.

.. important::

    It is important to note this notion of a String does not yet possess the attribute of length. It takes no stance on whether the entity under inspection is finite or infinite.

The set of all Strings is denoted :math:`S`. At this point, as noted, nothing definitive can be asserted about the contents or cardinality of :math:`S`. Once Characters are introduced and concatenation is defined, it will be possible to make claims regarding :math:`S`.

.. important::

    :math:`S` should be not understood as an entity with physical presence, i.e. it is not something that can be written down or listed. It is an abstraction of all possible linguistic constructions.

The goal is to establish the hierarchy of logical relations that govern Strings, Characters, Alphabets, Words, Languages, Sentences and Corpuses. As each of these entities is introduced and defined, a new level of relations will be revealed and codified. Palindromic symmetries will manifest on each level, in slightly different but related forms. Each type of symmetry will involve, in some form or another, the concept of :ref:`String Inversion <palindromics-string-inversion>`, to be defined shortly. The essence of a Palindrome lies in binding together the syntactical symmetries at every linguistic layer into a semantic whole. Indeed, it will be seen the symmetrical structure required by Palindromes in turn requires these linguistic layers to have explicit relations and specific synactical properties, regardless of their semantic interpretation.
...
The goal of this section is to describe the necessary syntactic conditions for a String to be considered a formal Word, without taking into account the semantic content that is assigned to it through everyday use. In other words, this section seeks to formally disentangle the syntactical functions of Words and Strings. 
...
A *Character* is the basic unit of a String. Characters will be represented as follows,
    ...
    1. Characters (:math:`\mathfrak{a}, \mathfrak{b},  \mathfrak{c}, ...`): Lowercase Fraktur letters represent Characters. Subscripts will occassionally be used in conjunction with Fraktur letters to denote Characters at specific positions within Strings, (:math:`\mathfrak{a}_1, \mathfrak{a}_2, \mathfrak{a}_3, ...`). 
    2. Empty (:math:`\varepsilon`): The lowercase Greek letter epsilon, :math:`\varepsilon`, represents the Empty Character.
    3. Delimiter (:math:`\sigma`): The lowercase Greek letter sigma, :math:`\sigma`, represents the Delimiter Character. 
...
The Empty Character also deserves special mention, since it represents a *null* Character. The Empty Character is understood as a Character with no semantic content. It can be concatenated into a String without altering it in any way. The domain of all Strings :math:`S`, as will be shown in (the albeit informal) :ref:`Theorem 1.1.2 <palindromics-theorem-1-1-2>`, is uncountably infinite. Beyond this, the Empty Character introduces further ambiguity when defining the concepts of Word and Language, since multiple Strings with the Empty Character, i.e. :math:`\mathfrak{a}\varepsilon, \mathfrak{a}\varepsilon\varepsilon, \mathfrak{a}\varepsilon\varepsilon\varepsilon, ...`, can represent the same semantic content. In order to formally define these linguistic entities, the Empty Character must be excluded from the domain of Words and Language. 

This point is often overlooked in formal language theory or other metamathematical treatments. In theories such as context-free grammar, it is tacitly assumed that language occurs at the level of syntax. Instead, as this work will argue, syntax should be understood as a *precondition* for language. As a consequence, as will be shown in :ref:`Theorem 6.1.1 <palindromics-theorem-6-1-1>`, the results of context free grammar with respect to palindromes are embedded directly within the logic of the current formal system.

:math:`S` is like :math:`\mathbb{R}`, the real numbers, in this respect. 
:math:`S` is "*chaotic*" and "*irrational*". If a String is conceived only as "*an ordering of Characters*", an infinite String cannot be assigned a representation in Language, just as irrational numbers cannot be assigned a finite decimal representation. The division of Strings and Words in linguistics is analogous to the division of Real Numbers and Natural Numbers in mathematics (however, this analogy is not to be taken *too* literally). :math:`S` cannot by itself be used as a domain on which to construct a formal language theory. This set must be "*filtered*" and "*prepared*" before it can be used to construct a Language. 

.. important::

    In other words, Strings and Language have syntactical properties that make them distinct sets beyond the semantic content that has been assigned to the elements of a Language.

Along these lines, at this early stage, it is has not yet been shown that Characters are Strings; the preceding statements should be taken heuristically. This will be rectified in the next section with the formal definition of concatenation and the introduction of :ref:`Character Axiom C.1 <palindromics-axiom-c1>`. 
...
It will sometimes be necessary to refer to indeterminate Characters, so notation is introduced for Character Variables,
    ...
    1. Character Variables (:math:`\iota, \nu, \omicron, \rho`): The Lowercase Greek letters Rho, Omicron, Iota and Nu will represent Character Variables, i.e. indeterminate Characters. Subscripts will occassionally be used with Iota to denote Character Variables, (:math:`\iota_1, \iota_2, \iota_3, ...` )
...
.. _palindromics-axiom-c0:

.. topic:: Axiom C.0: The Equality Axiom
...
Character Equality will be used to define *String Equality* in :ref:`Definition 1.1.4 <palindromics-definition-1-1-4>` and Word Equality in :ref:`Definition 1.2.3 <palindromics-definition-1-2-3>`.
...
.. _palindromics-definition-1-1-1:

**Definition 1.1.1: Concatenation**  

The result of *concatenating* any two Characters :math:`\iota` and :math:`\nu` is denoted :math:`\iota\nu`. To make the operands of concatenation clear, parenthesis will sometimes be used to separate the Characters being concatenated, e.g. :math:`\iota(\nu) = (\iota)\nu = (\iota)(\nu) = \iota\nu`. Character concatenation is defined inductively through the following schema,
    ...
    4. Comprehension Clause: :math:`\forall \iota \in \Sigma: \forall s \in S: {\iota}s \in S` 

∎

Colloquially, :math:`\iota\nu` is the result of placing :math:`\nu` behind :math:`\iota`.

The first clause in :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>` is the basis step of induction which states any Character appended to the Empty Character is the Character itself. The second clause is the inductive step which allows an arbitrary number of Characters to be recursively concatenated into Strings.

The Uniqueness Clause states that if the concatenation of two Characters :math:`\iota` and :math:`\nu` is equal to the concatenation of two separate Characters :math:`\omicron` and :math:`\rho`, then it must be the case that :math:`\iota` is equal to :math:`\omicron` and :math:`\nu` is equal to :math:`\rho`; there is one and only one set of Characters that yields the result of a concatenation .

It is assumed that the operation of concatenation is closed with respect to the set of all Strings :math:`S`, i.e. concatenation will always yield a String. This assumption is partly captured in the Comprehension and Inductive Clause of :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>`. The Inductive Clause breaks the concatenation of a sequence of Characters and a String down into a sequence of a concatenations between a single Character and a String (i.e., the concatenation of a sequence is a sequence of concatenations), whereas the Comprehension Claus ensures each concatenation in this sequence is a String.

However, these clauses alone do not ensure the closure of :math:`S` with respect to concatenation. In order to close :math:`S`, an additional assumption must be introduced. Before introducing this assumption in the form of an axiom, a brief explanation is required for this departure from convention.

Concatenation as it is normally found in the fields of automata theory or regular expressions is treated as a primitive operation performed between two string operands. Concatenation of multiple strings is then defined inductively, similar to :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>` but differing in the essential quality that it treats of only strings. The current formulation differs in that concatenation in this system is not conceived, at least in the primitive stage, as the "*joining*" of two or more Strings. Instead, the formal system under construction treats concatenation as an operation that occurs between Characters and Strings, i.e. it is a *hetergeneous* operation. Characters are concatenated *into* Strings.

:ref:`Definition 1.1.1 <palindromics-definition-1-1-1>` is a conscious decision to depart from convention. The reason for this distinction will become clear as the formal theory begins to detail palindromic structures that embody symmetry across linguistic levels. Strings are not primitive; This assumptions allows the expression of String properties beyond the concatenative algebra of regular expressions; in particular, it allows the formal system to make meaningful statements and assertions about the internal structure of Strings. The current formal system seeks to open the "*black box*" of Strings and provide greater insight into the syntactical condition that arise through their nature as ordered sequences of Characters.

To make this distinction plain, consider that given an Alphabet :math:`\Sigma` and :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>`, one still cannot say the result of a concatenation of two Characters is a String, nor make any claim about the contents of :math:`S`. The Comprehension Clause of :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>` only states the result of concatenating a Character with a String is a String. It says nothing at all about whether or not single Characters themselves are Strings, and thus it says nothing about whether the result of concatenating two or more Characters is itself a String. 

In order to rectify this, the first (official) Axiom is now introduced.

.. _palindromics-axiom-c1:
...
This Axiom states the intuitive notion that all Characters are Strings. This includes Empty Characters and Delimiter Characters. This Axiom, in conjunction with :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>`, immediately populates the set of all Strings :math:`S` with an uncountably infinite domain of objects (See :ref:`Theorem 1.1.2 <palindromics-theorem-1-1-2>` for an informal proof of this fact) consisting of every possible combination of Characters from the Alphabet, in every possible order. In other words, :ref:`Axiom C.1 <palindromics-axiom-c1>` in conjunction with :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>` ensure the domain is non-Empty for every non-trivial Alphabet.
...
By :ref:`Axiom C.1. <palindromics-axiom-c1>`, all Characters are Strings. By the Comprehension Clause of :ref:`concatenation <definition-1-1-1>`, a Character concatenated into a String is a String. Therefore, :math:`\mathfrak{ef} \in S`. As each nested concatenation is evaluated, the Inductive Clause ensures the next level of concatenation is a String. 

As a result, :math:`st = \mathfrak{abcdef}` and :math:`st \in S` 

∎

While this approach is atypical, this crucial distinction is the heart of the current system's insight: Characters and Strings (and Words and Sentences) are materially different linguistic entities.
...
A step-by-step breakdown of this algorithm is instructive for understanding how iterating through String is implemented at the most basic level in the theory of computation. Each command in this assembly-like language is broken down as follows,
    ...
    6. JE LOOP_END: If the comparison is equal, the program jumps to the LOOP_END label.
    7. ADD R1, 1: This moves the pointer to the next Character's address.
...
The Emptying Algorithm takes a string *t* as input, which can be thought of as a sequence of Characters :math:`\mathfrak{a}_1, \mathfrak{a}_2, \mathfrak{a}_3, ...`, where some characters might be :math:`\varepsilon`. It then initializes a set to hold :math:`T` and an index for the Characters it will add to :math:`T`. The algorithm iterates the index and constructs the Character-level representation by ignoring :math:`\varepsilon`. The Emptying Algorithm is formally defined below.

.. topic:: Algorithm Empty(t: String)

    # Input: A string :math:`t`

    # Output: An ordered set :math:`T` representing the character-level set representation of :math:`t`
    ...
    ## index for non-Empty Characters in :math:`T`
    
    .. math::

        j \leftarrow 1
    
    ## index for iterating through original String :math:`t`
    ...
    1. While :math:`\mathfrak{a}_i` exists:
        a. If :math:`\mathfrak{a}_i \neq \varepsilon` :
            i. :math:`T \leftarrow \{ (j, \mathfrak{a}_1) \cup T`
            ii. :math:`j \leftarrow j + 1`
        b. :math:`i \leftarrow i + 1`
    1. Return :math:`T`
...
Let *u* and *t* be arbitrary strings in :math:`S`. Let **U** and **T** be the character-level representations of *u* and *t*, respectively,
...
Let *ut* be the concatenation of *u* and *t*. By :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>`, the Character-level representation of *ut* is,
...
Since concatenation simply joins Characters without adding or removing Characters, with the possible exception of Empty Characters through the Basis Clause of :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>`, the non-Empty Characters in *ut* are precisely the non-Empty Characters from *u* followed by the non-Empty Characters from *t*.
...
With the concept of String Length now defined, it is also a simple matter to define String Equality in terms of Character Equality using the :ref:`Equality Axiom C.0 <palindromics-axiom-c0>`.

.. _palindromics-definition-1-1-4:
...
With the notion of String Length established for each element in the domain and some of its basic properties established, the size of the domain itself, :math:`S`, will now be elaborated in greater detail.
  
It is assumed :math:`S` is at least uncountably infinite. A rigorous proof of this fact would carry the current work too far into the realm of real analysis, but as motivation for this assumption, an informal proof is presented below based on Cantor's famous diagonalization argument. 

.. _palindromics-theorem-1-1-2:

**Theorem 1.1.2** :math:`\lvert S \rvert \geq \aleph_{1}`

Assume, for the sake of contradiction, that the set of all Strings :math:`S` is countable. This means the Strings can be listed in some order, 
...
This string *t* will be different from every string in :math:`S` contradicting the assumption that it was possible to list all strings. Therefore, :math:`S` must be uncountable. ∎ 
...
Let *s* be an arbitrary string in :math:`S`. By :ref:`Definition 1.1.3 <definition-1-1-3>`, :math:`l(\varepsilon) = 0`. Thus,
...
With these definitions, the hierarchy of relationships that exist between a Word *α*, its Language **L** and the set of all Strings :math:`S` is given by,
...
Let *a* and *b* be words in **L**. Then *a = b* if and only if *a* and *b* are equal as Strings (according to :ref:`Definition 1.1.4 <palindromics-definition-1-1-4>`). ∎ 
...
Therefore, *αε* and *εα* are both Strings with the same Characters as *α* in the same order. Since *α* is a Word in **L** and concatenation with *ε* does not change its length or order of Characters. Thus, by :ref:`Definition 1.2.3 <palindromics-definition-1-2-3>`, 
...
Since *u* and *s* have the same length (:math:`l(u) = l(t) = l(s)`) and the same Characters in the same order (:math:`u[j] = s[j]` for all *i*), by :ref:`Definition 1.1.4 <palindromics-definition-1-1-4>` of String Equality, it can be concluded that :math:`u = s`. Recall that :math:`u = \text{inv}(t)` and :math:`t = \text{inv}(s)`. Substituting, the desired result is obtained, :math:`\text{inv}(\text{inv}(s)) = s`. ∎ 
...
Let *u* and *t* be arbitrary strings in :math:`S`. Let :math:`l(u) = m` and :math:`l(t) = n`. Then, :math:`l(ut) = m + n`, by :ref:`Definition 1.1.3 <definition-1-1-3>`.

Let :math:`s = ut` . Let :math:`v = \text{inv}(s) = \text{inv}(ut)` . Let :math:`w = \text{inv}(t)\text{inv}(u)` .

To prove show the theorem, it must be shown that :math:`v = w`, which means, by :ref:`Definition 1.1.4 <palindromics-definition-1-1-4>`, it must be shown that 
...
.. math::
    ...
    f. \quad v[i] = w[i] (\text{since } w = \text{inv}(t)\text{inv}(u))

In both cases, :math:`v[i] = w[i]` for all :math:`i \in N_{l(v)}`. Since :math:`l(v) = l(w)`, by :ref:`Definition 1.1.4 <palindromics-definition-1-1-4>` it follows :math:`v = w`. Therefore, 
...
This theorem can be stated in natural language as follows: For any Strings *t*, *u*, and *v* in :math:`S`, if *t* is contained in *u* and *u* is contained in *v*, then *t* is contained in *v*.

Let *t*, *u*, and *v* be arbitrary Strings in :math:`S` such that both of the following expressions are true,
...
This theorem can be stated in natural language as follows: For every natural number n, and for every Phrase **P**:sub:`n` in the *n*:sup:`th` Lexicon of **L**, there exists a unique string *s* in :math:`S` such that *s* is the Delimitation of **P**:sub:`n`.
...
Thus, *s*:sub:`k` is unique. Since *α*:sub:`k+1` is a Word in **L** (by the definition of :math:`X_L (n+1)`), it is also a unique String in :math:`S`.

The concatenation of *s*:sub:`k`, *σ*, and *α*:sub:`k+1` is a unique string (by the :ref:`Definition 1.1.1 <palindromics-definition-1-1-1>` of Concatenation and :ref:`Definition 1.1.4 <palindromics-definition-1-1-4>` of String Equality).

Therefore, :math:`s_{k+1} = (s-k)(\sigma)(\alpha_{k+1})` is a unique string.

By induction, for every natural number *n*, and for every phrase **P**:sub:`n` in :math:`X_L (n)`, there exists a unique string *s* in :math:`S` such that
...
Since :math:`\beta[i] = \alpha[i]` for all :math:`i \in N_{l(\alpha)}`, and both strings have the same length, by :ref:`Definition 1.1.4 <palindromics-definition-1-1-4>`, it can be concluded that :math:`\alpha = \beta`. Therefore the desired result is obtained, :math:`\alpha = \beta = \text{inv}(\alpha)`.
...
Since both strings have the same length and the same Characters in the same order, by :ref:`Definition 1.1.4 <palindromics-definition-1-1-4>`, 
...
In the context of (potentially) infinite sets such as **L** and :math:`S`, *"even"* and *"odd"* refer to whether the set can be partitioned into two disjoint subsets of equal cardinality.
