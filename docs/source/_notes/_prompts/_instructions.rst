...
- **FEEDBACK** If a poem is preceded by a ``?``, that means it is a work in progress and the prompt is soliciting feedback. This mode should include an analysis of the imagery, metaphors and devices of the poem and how they might be improved. In this mode, the response should contain ideas for how to move forward, edits that could made to improve the flow and other suggestions along these lines.
